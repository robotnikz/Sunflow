...
const BatteryHealthWidget: React.FC<BatteryHealthWidgetProps> = ({ data, nominalCapacity }) => {
  ...
  // Calculate Health Metric based on Estimated Capacity vs Nominal
  const latestSohPercent = nominalCapacity > 0 ? (data.latestCapacityEst / nominalCapacity) * 100 : 0;
  
  let healthColor = 'text-emerald-400';
  let healthText = 'Excellent';
  
  // SOH Status logic
  if (latestSohPercent > 0) {
    if (latestSohPercent < 90) { healthColor = 'text-yellow-400'; healthText = 'Good'; }
    if (latestSohPercent < 80) { healthColor = 'text-amber-500'; healthText = 'Degrading'; }
    if (latestSohPercent < 70) { healthColor = 'text-red-500'; healthText = 'Poor'; }
  } else {
    // Fallback status if capacity estimation hasn't happened yet (0 cycles) but efficiency exists
    if (data.averageEfficiency > 90) { healthColor = 'text-emerald-400'; healthText = 'Excellent'; }
    else if (data.averageEfficiency > 80) { healthColor = 'text-yellow-400'; healthText = 'Good'; }
    else { healthColor = 'text-red-500'; healthText = 'Poor'; }
  }
      ...
      <div className="bg-slate-900 border border-slate-600 p-3 rounded-lg shadow-xl text-xs">
        ...
        {payload.map((entry: any, index: number) => (
             <div key={index} className="flex items-center justify-between gap-4 mb-1">
                ...
                <span className="font-mono text-slate-200">{Number(entry.value).toFixed(1)}{entry.unit}</span>
             </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="bg-slate-800 rounded-2xl p-6 border border-slate-700 shadow-lg relative overflow-hidden flex flex-col h-full">
        <div className="flex justify-between items-start mb-4">
            ...
            <div className={`px-2 py-0.5 rounded border bg-slate-900/50 flex items-center gap-1.5 ${healthColor} border-current opacity-80 shadow-sm transition-colors duration-500`}>
                <Battery size={12} />
                <span className="font-bold text-xs uppercase">{healthText}</span>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-slate-900/50 p-2.5 rounded-lg border border-slate-700/50">
                ...
                <div className="text-lg font-bold text-slate-100">{data.averageEfficiency.toFixed(1)}%</div>
            </div>
            <div className="bg-slate-900/50 p-2.5 rounded-lg border border-slate-700/50">
                 <div className="flex items-center gap-1.5 text-slate-400 text-[10px] font-bold uppercase mb-0.5">
        ...
        <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.dataPoints} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <Area yAxisId="left" type="monotone" dataKey="estimatedCapacity" name="Est. Capacity" unit=" kWh" stroke="#A855F7" fill="url(#colorCap)" connectNulls strokeWidth={2} />
                    <Area yAxisId="right" type="monotone" dataKey="efficiency" name="Efficiency" unit="%" stroke="#10B981" fill="none" connectNulls strokeWidth={2} strokeDasharray="4 4" />
                </AreaChart>
            </ResponsiveContainer>
        </div>
    </div>
